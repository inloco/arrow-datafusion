...
mod tests {
    ...
    macro_rules! test_coercion {
        ($A_ARRAY:ident, $A_TYPE:expr, $A_VEC:expr, $B_ARRAY:ident, $B_TYPE:expr, $B_VEC:expr, $OP:expr, $C_ARRAY:ident, $C_TYPE:expr, $VEC:expr) => {{
            ...
            let expression = binary(col("a"), $OP, col("b"), &dfschema)?;
    ...
    fn test_dictionary_type_to_array_coersion() -> Result<()> {
        ...
        // verify that we can construct the expression
        let expression = binary(col("dict"), Operator::Eq, col("str"), &dfschema)?;
        assert_eq!(expression.data_type(&dfschema)?, DataType::Boolean);
        ...
        // verify that we can construct the expression
        let expression = binary(col("str"), Operator::Eq, col("dict"), &dfschema)?;
        assert_eq!(expression.data_type(&dfschema)?, DataType::Boolean);
