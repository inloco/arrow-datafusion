...
pub fn merge_join_indices<'a>(
    ...
    while left_merge_cursor.row_index < left_size
        && right_merge_cursor.row_index < right_size
    {
    ...
    Ok((
        (left_merge_cursor.row_index, Arc::new(left_indices_array)),
        (right_merge_cursor.row_index, Arc::new(right_indices_array)),
    ))
}
...
#[cfg(test)]
mod tests {
    use crate::array::{ArrayRef, UInt64Array};
    use crate::compute::kernels::if_op::if_primitive;
    use crate::compute::kernels::merge::merge_sort_indices;
